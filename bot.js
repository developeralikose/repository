...
client.on('guildMemberAdd', member => {
  member.sendMessage("**Coes NetWork Sunucusuna Hoşgeldiniz Minecraft Sunucu IP: play.coesnetwork.com**")

});
// özel mesaj biter
//sunucudaki oyuncuları sayar
        ...
        if (küfür.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.hasPermission("BAN_MEMBERS")) {
                  msg.delete();
                    return msg.reply('Bu Sunucuda Küfür Engelleme Filtresi Aktiftir. Küfür Etmene İzin Veremem ! Yoksa Ali Abim sana Çoooook Kızar!!').then(msg => msg.delete(8000));
...
//ipreklamı
//setInterval(() => {
  //client.channels.get("664244721694146561").send('Serverimize hâla katılmadınızmı ? öyleyse hemen katılmaya ne dersin IP: Play.CoesNetwork.Online')
//}, 7200000)
//biter 
...
let role = await  db.fetch(`otorolisim_${member.guild.id}`)
 ...
 try {
  ...
  if (!i) return //CodeMareFi

  member.addRole(member.guild.roles.get(otorol))
                        var embed = new Discord.RichEmbed()
                        .setDescription(`**Sunucuya Yeni Katılan** \`${member.user.tag}\` **Kullanıcısına** \`${role}\` **Rolü verildi.\n Kayıt Olmak için !kayıt Oyundaki adını, yaşını ve ismini yazmalısın \n örnek: \n !kayıt GameLeader 18 Ali**`)
                        .setColor('0x36393E') //CodeMareFi
                        .setFooter(`Coes Kayıt sistemi`)
...
client.on('ready', () => {
    ...
    setInterval(() => {
     let channel = client.channels.find(c => c.id === '679243042481373194') //KANALID
     .bulkDelete(50);
     }, 3600000); 
...
client.on("message", message => {
    ...
    client.channels.get("682282151529676818").send(dmlog);
    }
});
//biter
...
client.on('guildBanAdd', async (guild, user) => {
  ...
  if(modlogs) {
    ...
    .addField(`Yasaklanan kişi`, `\`\`\` ${user.tag} \`\`\` `)
     .addField(`Yasaklanan kişi ID`, `\`\`\` ${user.id} \`\`\` `)
...
client.on('guildBanRemove', async (guild, user) => {
 ...
 .addField(`Yasağı kaldırılan kişi ID`, `\`\`\` ${user.id} \`\`\` `)
    .setFooter(`${botadi} | Mod-Log Sistemi`)
    .setTimestamp()
    modlogkanal.send(embed)
  }
...
client.on("userUpdate", async (oldUser, newUser) => {
  if (oldUser.username !== newUser.username) {
    let tag = "Coes"; //tagınız
    let sunucu = "664243908447830058"; //sunucu ID
    let kanal = "693641869032423475" //log kanal id
    let rol = "694618718663213166"; // rol ID
...
client.on('guildMemberAdd',async member => {
 const guild = member.guild
    ...
    if (kanalcık) {
      ...
      kanal.setName(`Toplam Oyuncu : ${guild.memberCount}`)
    }
})
